...
class BridgeRunner(L.LightningModule):
    ...
    def __init__(
        ...
        self.latent_dim = int(latent_dim or generator_params.get("nz", 10))
    ...
    def _predict_wavelet_x0(
        ...
        for _ in range(max(self.n_recursions, 1)):
            if return_aux:
                pred_next, aux_next = self.generator(gen_input, t, x_r=x0_r, return_aux=True,)
            else:
    ...
    def training_step(self, batch, batch_idx=None):
        ...
        x_t_cond = x_t.detach()
        disc_real = self.discriminator(x_prev_real_for_d, x_t_cond, t,)
        ...
        disc_fake = self.discriminator(x_prev_fake.detach(), x_t_cond, t,)
        ...
        adv_loss = self.adversarial_loss(
            self.discriminator(x_prev_fake, x_t.detach(), t,),
            is_real=True
        )
        c = self.base_channels
