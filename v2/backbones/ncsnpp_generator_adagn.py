...
class WaveletNCSNpp(NCSNpp):
    ...
    def __init__(self, config):
        ...
        self.use_cgfsi = bool(getattr(self.config, "use_cgfsi", False))
        self.cgfsi_use_edge_gate = bool(getattr(self.config, "cgfsi_use_edge_gate", True))
        self.cgfsi_init_gamma = float(getattr(self.config, "cgfsi_init_gamma", 0.0))

        # Target wavelet channels are [LL, LH, HL, HH].
        # cond_hf_ch is the channel count of [LH, HL, HH] from y_wavelet.
        out_wave_ch = int(getattr(self.config, "num_out_channels", self.config.num_channels))
        base_wave_ch = out_wave_ch // 4
        self.cond_hf_ch = int(getattr(self.config, "cond_hf_ch", 3))
        
        # Only for multi-scale auxiliary loss.
        # It does not change the main U-Net path.
        self.deep_supervision = bool(getattr(self.config, "deep_supervision", True))
        ...
        if resblock_type == 'ddpm':
            ...
            if self.no_use_freq:
                ...
                ResnetBlock = functools.partial(WaveletResnetBlockBigGAN,
                                                ...
                                                skip_rescale=skip_rescale,
                                                temb_dim=nf * 4,
                                                zemb_dim=z_emb_dim,use_cgfsi=self.use_cgfsi,
                                                cond_hf_ch=self.cond_hf_ch,
                                                cgfsi_use_edge_gate=self.cgfsi_use_edge_gate,
                                                cgfsi_init_gamma=self.cgfsi_init_gamma,)
        ...
        channels = config.num_channels * self.patch_size**2
        # Aux heads predict wavelet x0 at each decoder level.
        # In your current config patch_size=1, so aux_out_ch=num_out_channels is fine.
        aux_heads = []
        aux_out_ch = int(getattr(config, "num_out_channels", channels))
            ...
            if self.deep_supervision:
                aux_heads.append(
                    nn.Sequential(
                        ...
                        nn.SiLU(),
                        conv3x3(in_ch, aux_out_ch, init_scale=init_scale),
                    )
                )
    ...
    def forward(self, x, time_cond, z, return_aux: bool = False, cond_wavelet=None):
        cond_hf = None
        if self.use_cgfsi and cond_wavelet is not None:
            # cond_wavelet order: [LL, LH, HL, HH]
            base_c = cond_wavelet.shape[1] // 4
            cond_hf = cond_wavelet[:, base_c:, :, :]
        # patchify
        x = rearrange(x, "n c (h p1) (w p2) -> n (p1 p2 c) h w",
                      p1=self.patch_size, p2=self.patch_size)
        # timestep/noise_level embedding; only for continuous training
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if self.deep_supervision:
                if return_aux:
                    aux_h = self.aux_heads[aux_idx](h)
                    aux_outputs.append(aux_h)
                aux_idx += 1
            ...
            if i_level != 0:
                if self.resblock_type == 'ddpm':
                    ...
                    if self.no_use_freq:
                        ...
                        h = modules[m_idx](h, temb, zemb, skipH=skipHs.pop(),condH=cond_hf,)
                    m_idx += 1
        ...
        if return_aux:
            ...
            for aux in aux_outputs:
                # Your config uses patch_size=1. Keep this branch for safety.
                if self.patch_size != 1:
                    aux = rearrange(
                        aux,
                        "n (c p1 p2) h w -> n c (h p1) (w p2)",
                        p1=self.patch_size,
                        p2=self.patch_size,
                    )
                ...
                aux_processed.append(aux)

            return h, aux_processed
